-- Email commerciale v5: chi siamo + link al sito Attila Lab.
-- Le versioni sono immutabili: crea la v5 e sposta le campagne attive.
...
select
  ...
  '["business_name","city_phrase","preview_image_url","demo_url","cta_block","preview_image_block","offer_block","whatsapp_block","call_block","personalized_insight_block","studio_block","sender_name","offer_price","delivery_time"]'::jsonb
from public.message_templates mt
where mt.key = 'visual-intro-v1'
  and not exists (
    select 1
    from public.message_template_versions v
    where v.template_id = mt.id and v.version = 5
  );

update public.campaigns c
set message_template_version_id = v5.id
from public.message_templates mt
join public.message_template_versions v5
  on v5.template_id = mt.id and v5.version = 5
where mt.key = 'visual-intro-v1'
