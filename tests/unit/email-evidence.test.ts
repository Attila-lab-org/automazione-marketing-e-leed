...
describe('Email enrichment evidence — selected candidate provenance', () => {
  it('selected email evidence corresponds to selected candidate (not first page)', async () => {
    ...
    // First-page email must NOT steal provenance of the selected one
    const wix = result.candidateEvidence.find((c) => c.email === 'support@wix.com');
    expect(wix?.sourceUrl).not.toContain('/contatti');
  });
